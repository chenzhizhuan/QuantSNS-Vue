<template>
  <div class="ai-elastic-stock-page" :class="{ 'theme-dark': isDarkTheme }">

    <!-- ======== AI Trading Radar ======== -->
    <div class="radar-section" v-if="opportunities.length > 0">
      <div class="radar-header">
        <div class="radar-header-left">
          <h2 class="radar-title">{{ $t('aiElasticStock.opportunities.title') }}</h2>
          <p class="radar-subtitle">{{ $t('aiElasticStock.opportunities.updateHint') }}</p>
        </div>
...
const OPP_CACHE_KEY = 'aiElasticStock.opportunities'
...
export default {
  ...
  methods: {
    ...
    openQuickTradeFromCurrent () {
      ...
      this.qtSide = ''
      this.qtPrice = 0
      this.qtSource = 'ai_elastic_stock'
